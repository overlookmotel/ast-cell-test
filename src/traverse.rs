pub use crate::cell::TraverseCtx;
use crate::{
    ast::{
        traversable::{
...
pub fn transform<'a, T: Traverse<'a>>(transformer: &mut T, program: &mut Program<'a>) {
    ...
    let mut ctx = unsafe { TraverseCtx::new_unchecked() };
...
pub trait Traverse<'a> {
    ...
    fn visit_statement(&mut self, stmt: Statement<'a>, ctx: &mut TraverseCtx) {
        self.walk_statement(stmt, ctx)
    }

    fn walk_statement(&mut self, stmt: Statement<'a>, ctx: &mut TraverseCtx) {
        match stmt {
    ...
    fn visit_expression(&mut self, expr: Expression<'a>, ctx: &mut TraverseCtx) {
        self.walk_expression(expr, ctx);
    }

    fn walk_expression(&mut self, expr: Expression<'a>, ctx: &mut TraverseCtx) {
        match expr {
